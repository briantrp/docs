...
```bash
docker compose -f deployment/docker-compose.release.yml logs collabase-app
```
...
<Steps>
  <Step title="Check that the database service is running">
    ```bash
    docker compose -f deployment/docker-compose.release.yml ps collabase-postgres
    ```
    The status should be `running`. If not, start it:
    ```bash
    docker compose -f deployment/docker-compose.release.yml start collabase-postgres
    ```
  </Step>
...
<Steps>
  ...
  <Step title="Check the application logs">
    ```bash
    docker compose -f deployment/docker-compose.release.yml logs collabase-app | grep -i mail
    ```
...
### Cloud provider (OpenAI, Anthropic, Groq, Gemini)

<Steps>
  <Step title="Verify the API key">
    Go to **Admin → Settings → AI**. Re-enter the API key for your provider and click **Test Connection**. API keys expire or get revoked — generate a new one from your provider's dashboard if needed.
  </Step>
  <Step title="Check that Collabase Brain is toggled on">
    The master switch at the top of the AI settings page must be enabled. If it is off, all AI features are hidden from users.
  </Step>
...
<Steps>
  ...
  <Step title="Check the Ollama URL in Collabase">
    Go to **Admin → Settings → AI** and confirm the Ollama URL matches where Ollama is actually running. If Collabase is in Docker, use the host machine's IP address instead of `localhost` — e.g. `http://172.17.0.1:11434`.
  </Step>
</Steps>
